package com.zyhang.rxCollection

/**
 * ProjectName:RxCollection
 * Description:
 * Created by zyhang on 2017/10/30.下午2:18
 * Modify by:
 * Modify time:
 * Modify remark:
 */

interface Consumer3<T1, T2, T3> {

    @Throws(Exception::class)
    fun accept(t1: T1, t2: T2, t3: T3)
}

interface Consumer4<T1, T2, T3, T4> {

    @Throws(Exception::class)
    fun accept(t1: T1, t2: T2, t3: T3, t4: T4)
}

interface Consumer5<T1, T2, T3, T4, T5> {

    @Throws(Exception::class)
    fun accept(t1: T1, t2: T2, t3: T3, t4: T4, t5: T5)
}

interface Consumer6<T1, T2, T3, T4, T5, T6> {

    @Throws(Exception::class)
    fun accept(t1: T1, t2: T2, t3: T3, t4: T4, t5: T5, t6: T6)
}

interface Consumer7<T1, T2, T3, T4, T5, T6, T7> {

    @Throws(Exception::class)
    fun accept(t1: T1, t2: T2, t3: T3, t4: T4, t5: T5, t6: T6, t7: T7)
}

interface Consumer8<T1, T2, T3, T4, T5, T6, T7, T8> {

    @Throws(Exception::class)
    fun accept(t1: T1, t2: T2, t3: T3, t4: T4, t5: T5, t6: T6, t7: T7, t8: T8)
}

interface Consumer9<T1, T2, T3, T4, T5, T6, T7, T8, T9> {

    @Throws(Exception::class)
    fun accept(t1: T1, t2: T2, t3: T3, t4: T4, t5: T5, t6: T6, t7: T7, t8: T8, t9: T9)
}